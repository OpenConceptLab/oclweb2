...
class MappingHome extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    // Only refetch data when URL changes
    if (prevPath !== currentPath) {
      this.refreshDataByURL();
    }
    // Only highlight when search metadata changes
  ...
  getMappingURLFromPath() {
    ...
    if (scoped) return location.pathname;
    ...
    this.setState(
      ...
      () => {
        ...
        APIService.new()
          .overrideURL(URL)
          .get(null, null, {
            includeReferences: this.props.scoped === "collection",
          })
          .then((response) => {
            ...
            if (get(response, "detail") === "Not found.")
              ...
              this.setState(
                { isLoading: false, mapping: response.data },
                () => {
  ...
  highlightFromSearch = () =>
    ...
    setTimeout(
      () =>
        highlightTexts([
          { ...this.state.mapping, search_meta: this.props.searchMeta },
        ]),
      100,
    );
  ...
  getVersions() {
    APIService.new()
      ...
      .get(null, null, {
        includeCollectionVersions: true,
  ...
  getCollectionVersions() {
    this.setState({ isLoadingCollections: true }, () => {
      APIService.new()
        .overrideURL(
          this.getMappingURLFromPath() + "collection-versions/?limit=100",
        )
        .get()
        .then((response) => {
          this.setState({
  ...
  isVersionedObject() {
    return (
      this.props.global ||
      !this.props.match.params.mappingVersion ||
      this.state.mapping?.type === "Mapping"
    );
  }

  getContentMarginTop = () =>
    `${get(
      document.querySelector("header.resource-header.home-header"),
      "offsetHeight",
    ) || 95}px`;
  ...
  render() {
    ...
    return (
      <div style={isLoading ? { textAlign: "center", marginTop: "40px" } : {}}>
        ...
        {!isLoading && !hasError && (
          <div
            id="resource-item-container"
            className="col-xs-12 home-container no-side-padding"
          >
            <ScopeHeader
              {...headerParams}
            ...
            <div
              ...
              style={{
                position: "relative",
                marginTop: this.getContentMarginTop(),
              }}
            >
              <MappingHomeDetails
                scoped={this.props.scoped}
